...
object project3bonus extends App {
	...
	var idlePeer = "0"*numDigits
	var f = 0
	
	while(f<numFailures){
	  while(idlePeer=="0"*numDigits || Global.idleActorSet.contains(idlePeer)){
	    idlePeer = actorsArray(Random.nextInt(numNodes))
	  }
	  Thread.sleep(5)
	  Global.idleActorSet += idlePeer
	  f+=1
	}
	println("Failure peers are "+ Global.idleActorSet.size)
	...
	while(k<=numRequests){		
		for(sourceId<-actorsArray){
		  if(!Global.idleActorSet.contains(sourceId)){		  
			  ctr+=1
			  destinationId = sourceId
			  while(destinationId==sourceId || Global.idleActorSet.contains(destinationId)){
...
object Global{
  ...
  var idleActorSet:HashSet[String] = HashSet()
}
    ...
    case Route(key,source,hops)=>
      if(key==id){
        ...
        Global.actorMap(key) ! Route(key,source,hops+1)
	      ...
	      //  rtcol-=1
	      if(Global.idleActorSet.contains(routingTable(rtrow)(rtcol))&& rtcol!=0)
	        rtcol-=1
	      if(routingTable(rtrow)(rtcol)==null)
	        rtcol=0
	      //println(rtrow+" "+rtcol)
	      if(id=="00"&&rtcol==0){
	        rtcol+=1
	        //println("Destination cannot be reached")
	        //System.exit(0)
	      }
